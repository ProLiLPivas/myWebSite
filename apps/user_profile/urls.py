...
urlpatterns = [
    ...
    path('blacklist/<int:id>/api/', APIBlockUser.as_view(),),


    path('me/', MyProfile.as_view(), name='get_my_profile'),
    path('me/settings/api/' , APIProfileSettings.as_view() , ) ,
    path('me/subscribers/api/', APISubscribersList.as_view(),),
    path('me/subscriptions/api/', APISubscriptionsList.as_view(), ),
    path('me/friends/api/', APIFriendsList.as_view(), ),
    path('me/blacklist/api/', APIBlackList.as_view(),),
