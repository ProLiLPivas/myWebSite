...
class MessagesLit(View):
    ...
    def get(self, request):
        ...
        if request.is_ajax():
            chats_dict = ChatUtils.chats_to_list(request.user)
...
class SendMessage(View):
    ...
    def post(self, request):
        new_message = MessageUtils.create_message(request.POST, request.user.id)
        if new_message:
...
class UpdateMessage(View):
    def post(self, request):
        ...
        updated_message = MessageUtils.update_message(
...
class BaseChat:
    ...
    def get(self, request, id):
        ...
        if request.is_ajax():
            messages_dict, users_dict, chat_dict = ChatUtils.gen_chat_data(self.chat_connection.chat)
    ...
    def post(self, request, id):
        ...
        if request.POST['changes'] == 'add':
            return c.addUsers(request)
        elif request.POST['changes'] == 'kick':
            return c.removeUser(request)
        elif request.POST['changes'] == 'admin':
            return c.addRemoveAdmin(request)
        elif  request.POST['changes'] == 'name':
            return c.changeChatName(request)
        elif request.POST['changes'] == 'img':
            return c.changeChatImage(request)
        elif request.POST['changes'] == 'settings':
            return c.changeSettings(request)
        else:
...
class CreateChat(View):
    ...
    def get(self, request):
        ...
        if request.is_ajax():
            friends_dict = ChatUtils.generate_data_to_create_chat(request.user)
...
#         print(request.POST)
#         if bound_form.is_valid() and connection.role >= chat.send_messages:
#
