...
class ChatSerializer(serializers.ModelSerializer):
    ...
    def get_users(self, obj: Chat):
        connections = Connection2Chat.objects.filter(chat=obj)
        user_and_roles = [(con.user, con.role) for con in connections]
...
class ChatsListSerializer(ChatSerializer):
    ...
    def get_url(self, obj: Chat):
        return Connection2Chat.objects.get(
            chat=obj, user=self.context['request'].user).get_chat_url()
